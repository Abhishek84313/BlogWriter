...
- **FR-004**: The system MUST allow users to list their saved sessions, showing each session's unique identifier, original question, creation time, and latest update time.
...
- **FR-010**: The system MUST ensure that one user's saved sessions are not returned to another user.
...
- **SC-001**: A user can find and select any one of their 100 most recently updated saved sessions from the session list in under 30 seconds.
...
- Saved sessions are retained long term unless a future retention or deletion feature defines a different policy.
- The existing application identity can be used to isolate each user's saved sessions.
- Session listings initially cover the user's 100 most recently updated sessions; searching, filtering, deletion, and sharing are out of scope.
